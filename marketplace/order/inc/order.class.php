...
if (!$this->isNewID($this->fields['id'])) {
    // Añadir pestañas estándar sin restricciones
    $this->addDefaultFormTab($ong);

    // Mostrar Document_Item sin restricciones
    // La función 'addStandardTab' añade una pestaña estándar. Aquí aseguramos que siempre se muestre.
    // Eliminamos cualquier verificación de permisos.
    $options['force_no_permissions'] = true; // Añadir esta opción para deshabilitar la verificación de permisos
    ...
    public function showForm($ID, $options = [])
    {
        ...
        if (!$config->isConfigured()) {
            ...
            <div class='d-flex'>
                ...
                <div class='ms-2'>
                    " . __('You must set up at least the order life cycle before starting.', 'order') . " â€” $link
                </div>
            </div>
         </div>";
            return;
...
// Si el pedido está en estado entregado, entonces no permitir la edición
if ($this->isDelivered()) {
    $canedit = false;
}
...
if (!$this->getID()) {
    // Aquí puedes permitir la edición solo cuando se está creando el pedido
    if ($canedit) {
        $objectName = autoName(
            $this->fields["name"],
            "name",
        ...
        echo Html::input(
            ...
            [
                'value' => $objectName,
                'canedit' => true, // Permitir editar en la creación
            ]
        );
    } else {
    ...
    // Se está editando un pedido existente
    global $DB;
    ...
    if ($result === false) {
        ...
        if ($DB->numrows($result) > 0) {
            // Permitir edición del ID del caso solo para el perfil 4
            echo Html::input(
                'name',
                [
...
if ($ID > 0) {
    ...
    if ($result === false) {
        ...
        // Contar el número de perfiles encontrados
        $numProfiles = $DB->numrows($result);
        
        // Si el usuario tiene el perfil 4, se permite la edición
    ...
    // Se está creando un nuevo pedido, se permite la edición
    $objectOrder = autoName(
        $this->fields["num_order"],
        "num_order",
        ($template === "newcomp"),
...
echo "<td>" . __("Date of order", "order") . ":</td><td>";
    // Establecer la fecha actual automáticamente al crear el pedido
...
/* Área del pedido que se solicita */
echo "<tr class='tab_bg_1'><td>" . __("&Aacute;rea solicitada") . ": </td><td>";

// Verificar si se está creando un nuevo pedido
if (!$this->getID()) {
    // Aquí se permite editar solo al crear un pedido
    if ($canedit) {
    ...
    if ($result === false) {
        ...
        if ($DB->numrows($result) > 0) {
            // Permitir editar el área si el perfil es 4
            PluginOrderOrderType::Dropdown([
                'name'  => "plugin_order_ordertypes_id",
                'value' => $this->fields["plugin_order_ordertypes_id"]
            ]);
...
// Verificar si el usuario tiene el perfil con ID 30 para permisos de edición
global $DB;
$userID = Session::getLoginUserID();
...
if ($result === false) {
 ...
 if (!$this->getID()) {
        // Mostrar el estado por defecto como solo lectura con ícono y círculo de color
        ...
        if ($canedit && $DB->numrows($result) > 0) {
            // Mostrar el dropdown para editar el estado con ícono y círculo de color
            echo "<div style='display:flex; align-items:center;'>$stateIcon";
            ...
            // Mostrar el estado como solo lectura con ícono y círculo de color
            echo "<div style='display:flex; align-items:center;'>$stateIcon";
            echo Dropdown::getDropdownName("glpi_plugin_order_orderstates", $state);
            echo "</div>";
        }
...
/* Área del pedido que se solicita  //* location */
...
// Lógica para determinar si el campo debe ser editable
if (!$this->getID()) {
    // Se está creando un nuevo pedido
    if ($canedit) {
        // Mostrar el campo de selección de ubicación editable para todos los usuarios que crean pedidos
        Location::Dropdown([
            'name'   => "locations_id",
    ...
    if ($DB->numrows($resultProfile4) > 0) {
        ...
        // Mostrar la ubicación como solo lectura para los demás
...
if ($result === false) {
    ...
    if ($DB->numrows($result) > 0) {
        ...
        if ($this->fields['id'] === 0 || !$this->getID()) { // Asegúrate de que 'isSaved' sea una propiedad que determines cuando el pedido es nuevo
...
if ($result30 === false || $result4 === false) {
    ...
    // Verificar si se está creando un nuevo pedido
    if (!$this->getID()) {
        // Para nuevos pedidos, permitir la edición
        ...
        if ($DB->numrows($result30) > 0) {
            // Mostrar el comentario como texto sin permitir edición (solo lectura para el perfil 30)
            echo "<div class='form-control'>" . $this->fields["comment"] . "</div>";
        } elseif ($DB->numrows($result4) > 0) {
            // Permitir la edición solo para el perfil 4
            echo "<textarea cols='40' rows='3' name='comment' class='form-control'>" . $this->fields["comment"] . "</textarea>";
        } else {
            // Para los demás usuarios, mostrar como solo lectura
            echo "<div class='form-control'>" . $this->fields["comment"] . "</div>";
        }
    }
}
