...
abstract class CommonITILObject extends CommonDBTM
{
   ...
   // STATUS
    ...
    const CANCELLED = 13; // Cancelado (No Aplica)
...
public $requester_custom_name;
    ...
    public function getRequesterCustomName() {
        return $this->requester_custom_name;
    }

    public function setRequesterCustomName($name) {
        $this->requester_custom_name = $name;
    }
  ...
  if (isset($input['requester_custom_name'])) {
        // Asegúrate de que el valor sea seguro para almacenar o procesar
        $this->requester_custom_name = $input['requester_custom_name'];
    }
 return $input; // Asegúrate de devolver $input al final
        }
        ...
        if (isset($input["cancelleddate"]) && empty($input["cancelleddate"])) {
            unset($input["cancelleddate"]);
        }
...
if (
            in_array($this->fields["status"], $this->getCancelledStatusArray())
            && (in_array("date", $this->updates) || in_array("cancelleddate", $this->updates))
        ){
// closedate must be > create date
            if ($this->fields["cancelleddate"] < $this->fields["date"]) {
                Session::addMessageAfterRedirect(__('Invalid dates. Update cancelled.'), false, ERROR);
                if (($key = array_search('date', $this->updates)) !== false) {
                    unset($this->updates[$key]);
                    unset($this->oldvalues['date']);
                }
                if (($key = array_search('cancelleddate', $this->updates)) !== false) {
                    unset($this->updates[$key]);
                    unset($this->oldvalues['cancelleddate']);
                }
            }
...
public function computeDefaultValuesForAdd($input)
{
    ...
    if (in_array($input["status"], $this->getCancelledStatusArray())) {
        if (
            !isset($input["cancelleddate"])
            || $input["cancelleddate"] < $input["date"]
        ) {
            $input["cancelleddate"] = $input["date"];
        }
    }

    // Final validation for solvedate relative to closedate
    if (
        !isset($input["solvedate"])
        || $input["solvedate"] < $input["date"]
  ...
  public function computeCancelledDelayStat()
    {

        if (
            isset($this->fields['id'])
            && !empty($this->fields['date'])
            && !empty($this->fields['cancelleddate'])
        ) {
            $calendars_id = $this->getCalendar();
            $calendar     = new Calendar();
           ...
           // Using calendar
            if (
                ...
                return max(0, $calendar->getActiveTimeBetween(
                    $this->fields['date'],
                    $this->fields['cancelleddate']
                )
                                                             - $this->fields["waiting_duration"]);
            }
           // Not calendar defined
            return max(0, strtotime($this->fields['cancelleddate']) - strtotime($this->fields['date'])
                       - $this->fields["waiting_duration"]);
        }
        return 0;
    }
    ...
    public function showStatsDates()
    {
        ...
        if (in_array($this->fields['status'], $this->getCancelledStatusArray())) {
            echo "<tr class='tab_bg_2'><td>" . __('Cancelled date') . "</td>";
            echo "<td>" . Html::convDateTime($this->fields['cancelleddate']) . "</td></tr>";
        }
        echo "</table>";
    }
    ...
    public function showStatsTimes()
    {
       ...
       if (in_array($this->fields['status'], $this->getCancelledStatusArray())) {
            echo "<tr class='tab_bg_2'><td>" . __('Cancelled') . "</td><td>";
            if ($this->fields['cancelled_delay_stat'] > 0) {
                echo Html::timestampToString($this->fields['cancelled_delay_stat'], true, false);
            } else {
                echo '&nbsp;';
            }
            echo "</td></tr>";
    ...
    public function getUsedAuthorBetween($date1 = '', $date2 = '')
    {
        ...
        if (!empty($date1) || !empty($date2)) {
            $criteria['WHERE'][] = [
                'OR' => [
                    getDateCriteria("$ctable.date", $date1, $date2),
                    getDateCriteria("$ctable.cancelleddate", $date1, $date2),
                ]
            ];
        }
...
} else if ($item->fields['status'] == static::CANCELLED) {
                $fourth_col = sprintf(
                    __('Cancelled on %s'),
                    ($p['output_type'] == Search::HTML_OUTPUT ? '<br>' : '') .
                    Html::convDateTime($item->fields['cancelleddate'])
                );
...
// Verificar la interfaz del usuario
$interface = \Session::getCurrentInterface(); // Cambia aquí
        ...
        return [
            'NOT' => [
                "$table.status" => array_merge(
                    ...
                    static::getCancelledStatusArray()
                )
            ]
        ];
    }
