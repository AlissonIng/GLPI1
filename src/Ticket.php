...
public $requester_custom_name; 

public function __construct() {
    parent::__construct(); // Llama al constructor de la clase padre
    $this->requester_custom_name = ''; // Inicializa la variable
}

public function getFromDB($id) {
    // Llama al método de la clase padre
    if (parent::getFromDB($id)) {
        // Ahora puedes asignar $this->requester_custom_name aquí si ya está en la base de datos
        $this->requester_custom_name = $this->fields['requester_custom_name']; // Asegúrate de que 'requester_custom_name' esté en $fields
        return true;
    }
    return false;
}
  ...
  || isset($this->fields['status']) && in_array($this->fields['status'], array_merge($this->getClosedStatusArray(), $this->getCancelledStatusArray()))          ) {
            return false;
        }
...
{
    return [
        self::PLANNED   => ['link' => $this->getLinkURL() . '&_openfollowup=1'],
        self::WAITING   => ['link' => $this->getLinkURL() . '&_openfollowup=1'],
        self::CLOSED    => ['link' => $this->getLinkURL() . '&_openfollowup=1'],
        self::CANCELLED => ['link' => $this->getLinkURL() . '&_openfollowup=1']
    ];
}
public function getAllStatuses()
{
    $statuses = [
        self::INCOMING   => __('Nuevo'),
        self::ASSIGNED   => __('En curso (Asignado)'),
        self::PLANNED    => __('En curso (planificado)'),
        self::WAITING    => __('En espera'),
        self::CLOSED     => __('Cerrado'),
        self::CANCELLED  => __('Cancelado (No Aplica)'),
        // Agregar otros estados necesarios
    ];

    // Usamos getStatus para formatear cada estado
    foreach ($statuses as $key => $label) {
        $statuses[$key] = static::getStatus($key);
    }
    ...
    public function getAdditionalDatas()
    {
        ...
        $this->fields['requester_custom_name'] = $this->fields['requester_custom_name'] ?? '';
        }
    }
    ...
    if (!isset($input['requester_custom_name']) && isset($this->fields['requester_custom_name'])) {
        $input['requester_custom_name'] = $this->fields['requester_custom_name'];
    }
       //must be handled here for tickets. @see CommonITILObject::prepareInputForUpdate()
        ...
        $cat_id = $input['itilcategories_id'] ?? 20;
        if ($cat_id) {
...
// Asegúrate de que requester_custom_name esté accesible y no se pierda
$requesterName = $this->fields['requester_custom_name']; // Asegúrate de que este campo exista
...
$inquest->add(
    [
        ...
        'requester_name' => $requesterName, // Agrega el nombre del solicitante aquí
    ]
);
        }
    ...
    if ($input === false) {
        return false; // Si hay un error en la limpieza, retornamos false.
    }

    // Asegúrate de asignar el campo siempre
    $input['requester_custom_name'] = $input['requester_custom_name'] ?? ''; // Esto se asegurará de que siempre se muestre.
...
if ($this->canAddFollowups()) {
    ...
    echo $followup->showMassiveActionAddFollowupForm();
}
    }
...
*/
    ...
    public static function getTypes()
    {
        ...
        $options = [
            ...
            self::INCIDENT_TYPE => __('Incidente'),
    ...
    public static function getAllStatusArray($withmetaforsearch = false)
    {
       ...
       // To be overridden by class
        $tab = [self::INCOMING => _x('status', 'New'),
            ...
            self::CANCELLED   => _x('status', 'Cancelado (No Aplica)')
        ];
    ...
    public static function getCancelledStatusArray()
    {
    ...
    public static function showFormHelpdeskObserver($options = [])
    {
       ...
       //default values
        $ticket = new Ticket();
       $params = [
    'requester_name' => $_POST['requester_name'] ?? '', // Campo de texto para el nombre del solicitante
    'entities_id'        => $_SESSION["glpiactive_entity"],
    '_right'             => "all",
    'show_first'         => true,
...
$ticket->add($params);
    ...
    public function showForm($ID, array $options = [])
    {
       ...
       // Load template if available :
        ...
        if ($ID && in_array($this->fields['status'], array_merge($this->getClosedStatusArray(), $this->getCancelledStatusArray()))) {
            $canupdate = false;
            ...
            case "tovalidate": // tickets waiting for validation
                ...
                $WHERE = array_merge(
                    ...
                    [
                        ...
                        'NOT'                            => [
                            'glpi_tickets.status'   => [self::CLOSED, self::CANCELLED]
                        ]
                    ]
                );
                break;
            ...
            if ($showgrouptickets) {
                ...
                switch ($status) {
                    ...
                    /*case "toapprove":
                        ...
                        $options['criteria'][3]['searchtype'] = 'equals';
                        $options['criteria'][3]['value']      = 'notclosed';
                        $options['criteria'][3]['link']       = 'AND';
...
/*
    ...
    public static function cronInfo($name)
    {
        ...
        switch ($name) {
            ...
            case 'alertnotclosed':
                return ['description' => __('Not closed tickets')];
