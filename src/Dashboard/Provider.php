...
class Provider
{
     ...
     * - 'cancelled': casos en estado cancelado
     * @param array $params default values for
     * - 'title' of the card
     * - 'icon' of the card
     * - 'apply_filters' values from dashboard filters
           ...
           // Statuses speciale cases (no break)
            ...
            case 'cancelled':
                if (!$skip) {
                    ...
                    $params['icon']  = "fas fa-trashb";
                    $params['label'] = __("Casos cancelados");
                    $skip = true;
                }
             //no break
    ...
    public static function getTicketsStatus(array $params = []): array
    {
        ...
        $criteria = [
            'SELECT'   => [
                ...
                new QueryExpression(
                    "SUM(IF({$t_table}_distinct.status = " . Ticket::CANCELLED . ", 1, 0))
                  as " . $DB->quoteValue(_x('status', 'Cancelado'))
                ),
