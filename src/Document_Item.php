  ...
  public static function showForDocument(Document $doc)
{
    ...
    // Eliminamos la verificación de permisos
    // if (!$doc->can($instID, READ)) {
    //     return false;
    // }

    // Se asume que el documento siempre se puede editar
    $canedit = true; // Establecer en true para que no haya restricciones

    // Obtener tipos de elementos distintos
    ...
    foreach ($types_iterator as $type_row) {
        ...
        // Eliminamos la verificación de permisos para la vista
        // if ($item->canView()) {
        $iterator = self::getTypeItems($instID, $itemtype);

        foreach ($iterator as $data) {
            // Lógica existente para manejar el contenido
            $linkname_extra = "";
