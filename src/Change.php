...
{
    if (Session::haveRight('followup', CREATE) 
        && in_array($this->fields["status"], $this->getClosedStatusArray())
        && ($this->isAllowedStatus($this->fields['status'], self::INCOMING)
            || $this->isAllowedStatus($this->fields['status'], self::EVALUATION))) {

        // Preservar el nombre del solicitante al reabrir el ticket
        if (isset($this->fields['requester_custom_name'])) {
  ...
  public function prepareInputForAdd($input)
{
    ...
    // Preservar el nombre del solicitante si está definido
    if (isset($this->fields['requester_custom_name'])) {
    ...
    if (!isset($input['_skip_auto_assign']) || $input['_skip_auto_assign'] === false) {
        ...
        switch ($auto_assign_mode) {
            ...
            case Entity::AUTO_ASSIGN_CATEGORY_HARDWARE:
                // Auto asignar técnico/grupo desde la categoría de ITIL
...
public function prepareInputForUpdate($input)
{
    ...
    // Mantener el nombre del solicitante si está definido
    if (isset($this->fields['requester_custom_name'])) {
        $input['requester_custom_name'] = $this->fields['requester_custom_name'];
    }
    ...
    public static function getAllStatusArray($withmetaforsearch = false)
    {
        ...
        $tab = [self::INCOMING      => _x('status', 'New'),
            ...
            self::CANCELLED      => _x('status', 'Cancelado'),
