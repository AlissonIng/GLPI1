...
if ($nb_answersatisfaction > 0) {
    // Calcular el total de satisfacción
    $totalSatisfaction = array_sum($satisfaction);
    $avgsatisfaction = round($totalSatisfaction / $nb_answersatisfaction, 0); // Redondeo al número entero más cercano
    
    // Definir la carita según el promedio de satisfacción
    if ($avgsatisfaction == 3) {
        $emoji = '<span style="font-size: 30px;">&#128522;</span>'; // Carita feliz grande
    } elseif ($avgsatisfaction == 2) {
        $emoji = '<span style="font-size: 30px;">&#128528;</span>'; // Carita neutral grande
    } elseif ($avgsatisfaction == 1) {
        $emoji = '<span style="font-size: 30px;">&#128577;</span>'; // Carita triste grande
    } else {
        $emoji = '&nbsp;'; // Si no coincide con ninguna de las opciones, espacio en blanco
    }
} else {
    $emoji = '&nbsp;'; // Sin respuesta
}
// Mostrar la carita de satisfacción
echo Search::showItem($output_type, $emoji, $item_num, $row_num);
